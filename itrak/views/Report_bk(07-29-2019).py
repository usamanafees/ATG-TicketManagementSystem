        ...
        if report_id in reportdict:
            if report_id == 'IssuesPriority':
                ...
                tickets_closed = Ticket.objects.values('priority__priority_name').filter(
                    priority_id__isnull=False).filter(ticket_closed_at__gte=start_date).filter(
                    ticket_closed_at__lte=end_date).annotate(
                    tcount=Count('ticket_id'))
...
def reportWriterQueries(request):
    ...
    qb_queries = SavedQBQuries.objects.filter(qb_query_is_delete=0)
    print(qb_queries)
    context = {
        'sidebar': load_sidebar,
...
def thirdQuery(request):
    ...
    for field in selected_fields:
        try:
            actual_field = DataSetsPairFields.objects.only('df_actual_column_name').get(
                df_name=field).df_actual_column_name
        except:
            return render_to_response('itrak/page-404.html')
    ...
    if request.is_ajax() and request.method == 'POST':
        if request.POST.get('expression'):
            ...
            for filter_exp in expression_content:
                ...
                try:
                    fieldType = DataSetsPairFields.objects.only('df_condition_type').get(
                        df_actual_column_name=filter_exp_dict['modelAttr']).df_condition_type
                except:
                    return render_to_response('itrak/page-404.html')
    ...
    with transaction.atomic():
        if request.method == 'POST':
            ...
            if 'description' in request.POST and request.POST['description']:
                ...
                description = ''
            filter_expression_array = request.session['filter_expression_array']
            filter_statement = request.session['filter_statement'].split(" ") if request.session[
                'filter_statement'] else ''
            selected_fields = request.session['selected_fields_array']
            ...
            obj = SavedQBQuries(
                ...
                qb_filter_statement=json.dumps(filter_statement),
            ...
            return redirect("/Home_GetReportWriter")
        else:
...
@active_user_required
def qbQueryProcess(request):
    query_id = request.GET.get('qbQuery')
    obj = SavedQBQuries.objects.get(pk=query_id)
    qb_filter_statement = json.loads(obj.qb_filter_statement)
    qb_filter_expression_array = json.loads(obj.qb_filter_expression_array)
    qb_selected_fields = json.loads(obj.qb_selected_fields)

    request.session['filter_expression_array'] = qb_filter_expression_array
    request.session['filter_statement'] = ' '.join(qb_filter_statement) if len(qb_filter_statement) > 0 else ''
    request.session['selected_fields_array'] = qb_selected_fields
