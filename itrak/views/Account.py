...
def addAccount(request):
    ...
    return render(request, 'itrak/Account/account_add.html', context)

# Account Add Request End#


# Account Save Request Start#
@active_user_required
def saveAccount(request):
    if request.user.admin != 1:
        return render(request, 'itrak\page-404.html')
    if request.method == 'POST':
        AIAN_DK = request.POST.get('AIAN_DK')
        ...
        query="Insert into GlobalACCTS(AIAN_DK, acc_number, acc_name, client, company, country, agency, login_id, agency_email, currency, status, note, created_at, updated_at) values(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, getdate(), getdate())"
        data_tuple=(AIAN_DK, acc_number, acc_name, client, company, country, agency, login_id, agency_email, currency, status, note )
        cursor.execute(query,data_tuple)

        messages.success(request, 'Request Succeed! Account added.')
        return redirect('addAccount')
    else:
        # return HttpResponse('Fail')
        messages.error(request, 'Request Failed! Account cannot be added.Please try again.')
        return redirect('addAccount')
# Account Save Request Start#


# Account List Request Start#
@active_user_required
def listAccount(request):
    ...
    return render(request, 'itrak/Account/account_list.html', context)
# Account List Request End#
...
# Get Account List Json Request Start#
@active_user_required
def getAccountListJson(request):
    ...
    allAccountsArray = []
    for index, tuple in enumerate(allAccounts):
        innerArray = {}
        innerArray['AIAN_DK'] = tuple[1]
        innerArray['acc_number'] = tuple[2]
        innerArray['acc_name'] = tuple[3]
        innerArray['agency'] = tuple[4]
        innerArray['login_id'] = tuple[5]
        innerArray['agency_email'] = tuple[6]
        if tuple[7] == True:
            innerArray['status']  = "Active"
        else:
            innerArray['status'] = "Inactive"
        innerArray['country'] = tuple[8]
        innerArray['client'] = tuple[9]
        innerArray['company'] = tuple[10]
        innerArray['currency'] = tuple[11]
        allAccountsArray.append(innerArray)
    return HttpResponse(json.dumps(allAccountsArray), content_type="application/json")
# Get Account List Json Request End#
