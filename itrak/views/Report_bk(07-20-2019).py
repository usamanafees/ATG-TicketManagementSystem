...
def getCompoundFilterForQuery(filter_statement, filter_expression_array, conditiondict):
    ...
    compound_filter = ''
    tempcompound_filter = []
    remain_filter_statement = list(filter_statement) #filter_statement.copy() can also be  used but restrict to Python 3

    for key in filter_statement:
        remain_filter_statement.pop(0)
        print(key)
        ...
        try:
            if len(key) == 1:
                if key == '(':
                    print(argument_list)
                    print(compound_filter)
                    filter_statement = remain_filter_statement
                    tempcompound_filter.append(compound_filter)
                    argument_list.append(getCompoundFilterForQuery(filter_statement, filter_expression_array, conditiondict))
