 ...
 *	jQuery Dropdown1
 *
 *	https://github.com/daneWilliams/jquery.dropdown1
 *
 *	================================================================
 *
 *	@version		2.2.0
...
;(function( $, window, document, undefined ) {
	...
	function Dropdown1( elem, options ) {
		...
		// Set options
		this.opt = $.extend( true, {}, defaults, options, $(elem).data('dropdown1') );
	...
	$.extend( Dropdown1.prototype, {
		...
		getMenu: function( menu ) {
			...
			if ( typeof menu === 'string' ) {
				...
				// Element
				else if ( elem.dropdown1.find( '#' + menu ) )
					menu = elem.dropdown1.find( '#' + menu );
			...
			if ( menu.jquery ) {
				...
				var uid = menu.data( 'dropdown1-uid' );
		...
		getItem: function( item ) {
			...
			if ( typeof item === 'string' ) {
				...
				// Element
				else if ( elem.dropdown1.find( '#' + item ) )
					item = elem.dropdown1.find( '#' + item );
			...
			if ( item.jquery ) {
				...
				var uid = item.data( 'dropdown1-uid' );
		...
		toggleText: function( text ) {
			...
			// Store original
			if ( !$toggle.data( 'dropdown1-text' ) )
				$toggle.data( 'dropdown1-text', $text.html() );

			// Reset
			if ( !text ) {

				$text.html( $toggle.data( 'dropdown1-text' ) );
				return true;
		...
		toggleTextMulti: function( text ) {
			...
			if ( !text ) {
				...
				$toggle.data( 'dropdown1-text-multi', [] );
				$text.html( $toggle.data( 'dropdown1-text' ) );
			...
			// Get values
			var vals = $toggle.data( 'dropdown1-text-multi' );
			    vals = ( vals ? vals : [] );
			...
			// Get new text
			var str = $toggle.data( 'dropdown1-text' );

			if ( vals.length )
				str = vals.join( ', ' );

			// Store values
			$toggle.data( 'dropdown1-text-multi', vals );
		...
		_collisionValues: function( menu, resize ) {
			...
			collision.offset = {
				x: elem.dropdown1.offset().left,
				y: elem.dropdown1.offset().top
			};
		...
		_bind: function() {
			...
			// Select item
			elem.dropdown1.on( 'click', '.' + self._cls.menuItem, function(e) {

				e.preventDefault();

				self.select( $(this).data( 'dropdown1-uid' ) );
			...
			// Open dropdown1
			elem.dropdown1.on( 'dropdown1.open', function() {
			...
			// Close dropdown1
			elem.dropdown1.on( 'dropdown1.close', function() {

				self.close();

			});

			elem.dropdown1.on( 'click', '.' + self._cls.closeButton, function(e) {
			...
			// Back
			elem.dropdown1.on( 'click', '.' + self._cls.backButton, function(e) {
			...
			if ( opt.autoClose ) {
				...
				$(document).on( 'mousedown', function(e) {
					...
					var $target   = $(e.target);
					var $dropdown1 = $target.parents( '.' + self._cls.dropdown1 );

					if ( ( !$dropdown1.length || $dropdown1.data('uid') != inst.uid ) )
						self.close();
		...
		_build: function() {
			...
			// Build the structure
			elem.overlay.appendTo( elem.dropdown1 );
			elem.menuWrapper.appendTo( elem.dropdown1 );
			elem.menuContainer.appendTo( elem.menuWrapper );

			elem.toggleButton.prependTo( elem.dropdown1 );
			...
			// Add to page
			self.$elem.hide().after( elem.dropdown1 );
		...
		_buildMenu: function( menu ) {
			...
			// Add ID
			$menu.data( 'dropdown1-uid', menu.uid );
		...
		_buildItem: function( item ) {
			...
			// Add ID
			$item.data( 'dropdown1-uid', item.uid );
		...
		_populateSelect: function( $target ) {
			...
			$target.children().each(function() {
				...
				// UID
				if ( $this.data('dropdown1-uid') )
					item.uid = $this.data('dropdown1-uid');
		...
		_populateList: function( $target ) {
			...
			$target.children().each(function() {
				...
				if ( $this.children('ul, ol').length ) {
					...
					item.text = $this.data('dropdown1-text');
					...
					item.text  = $this.text();
					item.value = $this.data('dropdown1-value');

					if ( $this.data('dropdown1-text') )
						item.text = $this.data('dropdown1-text');
					...
					// URL
					if ( $this.data('dropdown1-url') )
						item.url = $this.data('dropdown1-url');

					// Selected
					if ( $this.data('dropdown1-selected') || $this.hasClass( self._cls.selected ) )
						item.selected = true;
		...
		/**
		 *
		 *	Fired before the dropdown1 is opened
		 *
		 *	================================================================ */
		...
		_beforeOpen: function() {
			...
			// Update dropdown1
			elem.dropdown1.addClass( self.cls.opening );
			elem.dropdown1.addClass( self.cls.animating );
			...
			if ( opt.collision ) {
				...
				if ( resize.collision.position.y == 'top' ) {
					...
					elem.dropdown1.removeClass( self.cls.below );
					elem.dropdown1.addClass( self.cls.above );

					self.inst.above = true;

				} else {

					elem.dropdown1.removeClass( self.cls.above );
					elem.dropdown1.addClass( self.cls.below );
				...
				if ( !elem.dropdown1.hasClass( '.' + self._cls.below ) && !elem.dropdown1.hasClass( '.' + self._cls.above ) )
					elem.dropdown1.addClass( self.cls.below );
		...
		/**
		 *
		 *	Fired after the dropdown1 is opened
		 *
		 *	================================================================ */
		...
		_afterOpen: function() {
			...
			// Update dropdown1
			elem.dropdown1.addClass( cls.open );
			elem.dropdown1.removeClass( cls.opening );
			elem.dropdown1.removeClass( cls.animating );
		...
		/**
		 *
		 *	Fired before the dropdown1 is closed
		 *
		 *	================================================================ */
		...
		_beforeClose: function() {
			...
			// Update dropdown1
			elem.dropdown1.addClass( cls.animating );
			elem.dropdown1.addClass( cls.closing );
		...
		/**
		 *
		 *	Fired after the dropdown1 is closed
		 *
		 *	================================================================ */
		...
		_afterClose: function() {
			...
			// Update dropdown1
			elem.dropdown1.removeClass( cls.animating );
			elem.dropdown1.removeClass( cls.closing );
			elem.dropdown1.removeClass( cls.open );
		...
		/**
		 *
		 *	Fired before the dropdown1 is resized
		 *
		 *	================================================================ */
		...
		_beforeResize: function( menu ) {
			...
			// Update dropdown1
			elem.dropdown1.addClass( cls.resizing );
		...
		/**
		 *
		 *	Fired after the dropdown1 is resized
		 *
		 *	================================================================ */
		...
		_afterResize: function( menu ) {
			...
			// Update dropdown1
			elem.dropdown1.removeClass( cls.resizing );
		...
		/**
		 *
		 *	Called before the dropdown1 is resized
		 *
		 *	================================================================ */
		...
		/**
		 *
		 *	Called after the dropdown1 is reset
		 *
		 *	================================================================ */
	...
	var classes = {
		...
		// Dropdown1
		dropdown1:		'dropdown1',
		overlay:		'dropdown1-overlay',

		// Menu
		menuMain:		'dropdown1-menu-main',
		menuOpen:		'dropdown1-menu-open',

		menuWrapper:	'dropdown1-menu-wrapper',
		menuContainer:	'dropdown1-menu-container',
		menuObject:		'dropdown1-menu',
		menuMask:		'dropdown1-mask',

		menuHeader: 	'dropdown1-header',
		menuTitle:		'dropdown1-title',

		menuList:		'dropdown1-list',
		menuItem:		'dropdown1-item',
		menuLink:		'dropdown1-link',
		menuText:		'dropdown1-text',
		menuParent:		'dropdown1-parent',

		menuDivider:	'dropdown1-divider',
		menuLabel:		'dropdown1-label',

		// Toggle
		toggleButton:	'dropdown1-toggle',
		toggleText:		'dropdown1-text',
		toggleIcon:		'dropdown1-icon',

		// Close
		closeButton:	'dropdown1-close',
		closeText:		'dropdown1-text',
		closeIcon:		'dropdown1-icon',

		// Back
		backButton:		'dropdown1-back',
		backText:		'dropdown1-text',
		backIcon:		'dropdown1-icon',

		// States
		open:			'dropdown1-open',
		opening:		'dropdown1-opening',
		closing:		'dropdown1-closing',
		focused:		'dropdown1-focused',
		animating:		'dropdown1-animating',
		resizing:		'dropdown1-resizing',
		selected:		'dropdown1-selected',
		selectedParent:	'dropdown1-parent-selected',

		// Position
		above:			'dropdown1-above',
		below:			'dropdown1-below'
	...
	$.fn.dropdown1 = function( options ) {
		...
		if ( options === undefined || typeof options === 'object' ) {
			...
			return this.each( function() {
				...
				if ( !$.data( this, 'plugin.dropdown1' ) ) {

					$.data( this, 'plugin.dropdown1', new Dropdown1( this, options ) );
				}
			...
			this.each( function() {
				...
				var instance = $.data( this, 'plugin.dropdown1' );

				// Allow access to public methods
				if ( instance instanceof dropdown1 && typeof instance[ options ] === 'function' ) {
					returns = instance[ options ].apply( instance, Array.prototype.slice.call( args, 1 ) );
				}

				// Allow instances to be destroyed via the 'destroy' method
				if ( options === 'destroy' ) {
					$.data( this, 'plugin.dropdown1', null );
				}
